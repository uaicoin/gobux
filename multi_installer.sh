...
# Uaicoin Multi-installer
# a one line clone-and-compile for uaicoin:
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/uaicoin ]; then
            read -r -p "${1:-uaicoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old uaicoin directory" && echo
                    rm -rf "$PWD"/uaicoin
                    ;;
                *)
                    _fail "uaicoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir uaicoin
        git clone -q https://github.com/uaicoin/uaicoin uaicoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd uaicoin
    fi
}

_build_uaicoin() {
    _note "Building uaicoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/uaicoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/uaicoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "uaicoin Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'uaicoin/build/src/' for the executible binaries. See 'https://github.com/uaicoin/uaicoin' for more project support. Cowabunga!"
